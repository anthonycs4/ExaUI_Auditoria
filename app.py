...
def obtener_riesgos(activo):
    print(f"⚙️ Llamando a Ollama para obtener riesgos de: {activo}")
...
• **Fallos hardware**: daño debido a causas como sobrecalentamiento, sobrecarga eléctrica o errores en la manufactura, lo que lleva a una pérdida de datos o inoperatividad del sistema."""},
    ...
    try:
        ...
        response.raise_for_status()  # Lanza una excepción si la respuesta no fue 200

        answer = response.json()["choices"][0]["message"]["content"]
        print("Respuesta del modelo:", answer)

        # Usar expresión regular para extraer riesgos
        patron = r'• \*\*(.+?)\*\*: (.+?)(?:\n|$)'
